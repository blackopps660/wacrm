...
// Ported from src/lib/dashboard/queries.ts (web app) — same queries,
// same RPCs, same RLS scoping (every query implicitly scoped to the
// signed-in user's account, same as web). No Next.js coupling in the
// original, so this is a straight port.
...
export async function loadResponseTime(db: DB): Promise<ResponseTimeSummary> {
  ...
  const samples: Sample[] = ((data ?? []) as { customer_at: string; response_at: string }[]).map(
    (row) => ({
